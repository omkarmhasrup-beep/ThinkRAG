...
t1 = time.time()
res = requests.post(f'http://localhost:5173/api/messages/{chat_id}/generate', json={'role': 'user', 'content': 'what is artificial intelligence?'}, headers=headers, stream=True)
first = True
for chunk in res.iter_content(chunk_size=1):
    if first:
        print(f'TTFT (Proxy): {time.time()-t1:.2f}s')
        first = False
print(f'Total (Proxy): {time.time()-t1:.2f}s')
